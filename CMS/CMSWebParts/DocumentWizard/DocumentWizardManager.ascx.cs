...
public partial class CMSWebParts_DocumentWizard_DocumentWizardManager : CMSAbstractWebPart, IDocumentWizardManager
{
    ...
    private bool LoadSteps()
    {
        ...
        if (!currentPath.StartsWithCSafe(path.TrimEnd('%')))
        {
            StopProcessing = true;
            return false;
        }
        ...
        if (!DataHelper.DataSourceIsEmpty(data))
        {
            ...
            foreach (DataRowView dr in ds)
            {
                ...
                if ((CurrentStep == null) && (currentPath.Equals(Convert.ToString(dr["NodeAliasPath"]), StringComparison.InvariantCultureIgnoreCase)))
                {
                    CurrentStep = step;
                }
