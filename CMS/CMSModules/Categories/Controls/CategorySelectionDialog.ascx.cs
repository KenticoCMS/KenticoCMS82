...
public partial class CMSModules_Categories_Controls_CategorySelectionDialog : CMSUserControl, ICallbackEventHandler
{
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        canModifyGlobal = AllowGlobalCategories && MembershipContext.AuthenticatedUser.IsAuthorizedPerResource("CMS.Categories", "GlobalModify");
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        switch (SelectionMode)
        {
            ...
            case SelectionModeEnum.MultipleTextBox:
                if (allowEditTextBox)
                {
                    buttonsScript += "function US_Submit(){ SelectItems(escape(ItemsElem().value), escape(ItemsElem().value.replace(/^" + ValuesSeparator + "+|" + ValuesSeparator + "+$/g, '')), " + ScriptHelper.GetString(hdnClientId) + ", " + ScriptHelper.GetString(txtClientId) + ", " + ScriptHelper.GetString(hashId) + ", HashElem().value); return false; }";
                }
                else
                {
        ...
        string encodedParentClientId = ScriptHelper.GetString(parentClientId, false);
            ...
            case SelectionModeEnum.MultipleButton:
                {
                    ...
                    {
                        script.AppendFormat("function SelectItems(items,hash) {{ wopener.US_SelectItems_{0}(items,hash); CloseDialog(); }}", encodedParentClientId);
                    }
                    else
                    {
...
function SelectItems(items, names, hiddenFieldId, txtClientId, hashClientId, hash) {
    ...
    }", (fireOnChanged ? "wopener.US_SelectionChanged_" + encodedParentClientId + "();" : ""), @"
    return CloseDialog(); 
}
...
function SelectItemsReload(items, names, hiddenFieldId, txtClientId, hidValue, hashClientId, hash) {
    ...
    wopener.US_ReloadPage_", encodedParentClientId, @"();
    return CloseDialog();
}");
                }
                break;
...
function Cancel() { 
    wopener.US_RefreshPage_", encodedParentClientId, @"(); CloseDialog(); 
}
...
function SelectAllItems(checkbox, hash) {
    ...
    ProcessItem(null, hash, false, true);
        ...
        treeScript += "return confirm(" + ScriptHelper.GetLocalizedString("general.confirmdelete") + ");";
        treeScript += "}";
    ...
    private UniTreeProvider CreateTreeProvider(int siteId, int userId)
    {
        ...
        if (!string.IsNullOrEmpty(hidItem.Value))
        {
            ...
            if (returnColumnName == "CategoryID")
            {
                int[] valIds = ValidationHelper.GetIntegers(hidItem.Value.Split(new[] { ValuesSeparator }, StringSplitOptions.RemoveEmptyEntries), 0);
                selected = "0";
