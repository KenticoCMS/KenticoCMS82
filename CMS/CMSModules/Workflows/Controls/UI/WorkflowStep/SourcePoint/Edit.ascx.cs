        ...
        if (Visible && !StopProcessing)
        {
            if (!validateData || ValidateData())
            {
                if (CurrentSourcePoint == null)
                {
                    ...
                    {
                        URLHelper.Redirect(URLHelper.AppendQuery(AfterCreateRedirectURL, String.Format("?workflowstepid={0}&sourcepointGuid={1}&saved=1", CurrentStepInfo.StepID, SourcePointGuid)));
                    }
                }
                else
                {
                    ...
                    WorkflowHelper.RefreshDesignerFromDialog(Page, CurrentStepInfo.StepID, QueryHelper.GetString("graph", String.Empty));
                }               
            }
        }
    }
