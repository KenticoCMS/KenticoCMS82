                ...
                if(!ignoreAttachedElements){
                    if(hover) {
                        if(this._jsPlumb.hoverItem){
                            $cmsj(this._jsPlumb.hoverItem.canvas).mouseleave();
                        }
                        this._jsPlumb.hoverItem = this;
                        $cmsj(self.canvas).addClass(self._jsPlumb.hoverClass);
                        if(self.tooltip) {
                            $cmsj(self.canvas).tooltip().show();
                        }
                    } else {
                        this._jsPlumb.hoverItem = null;
                        $cmsj(self.canvas).removeClass(self._jsPlumb.hoverClass);
                        if(self.tooltip) {
                            $cmsj(self.canvas).tooltip().hide();
                        }
                    }
                }
                /*CMS EDIT END*/

                if (self.hoverPaintStyle != null && !$cmsj(self.canvas).hasClass("Selected")) {
    ...
    var jsPlumbInstance = function(_defaults) {
        ...
        */
        ...
        _newConnection = function(params) {
            ...
            $cmsj(con.canvas).data('jsPlumbObject', con); //CMS EDIT - to support handlers over endpoints
            return con;
        },
        ...
        _newEndpoint = function(params) {
            ...
            $cmsj(ep.canvas).data('jsPlumbObject', ep); //CMS EDIT - to support handlers over endpoints
            return ep;
        },
        ...
        */
        ...
        /*
        * CMS EDIT: added this class to easily distinct hover elements
        * Property: hoverClass 
        * The CSS class to set on an hover item.
        */
            ...
            */
            ...
            this.createReattachHelper = function(el, externalAnchors) {                    
                ...
                if(self.endpoints[1].tooltip){
                    var configuration = $cmsj(self.endpoints[0].canvas).tooltip().getConf();
                    $cmsj(self.endpoints[1].canvas).tooltip(configuration);
                }
            ...
            // CMS EDIT - binding listener to connector instead of connection
            this.bind = function (event, listener) {
                self.connector.bind(event, listener);
            }
            /*CMS EDIT*/
            ...
            */
            ...
            if (params.isSource && jsPlumb.CurrentLibrary.isDragSupported(_element)) {
                ...
                var start = function() {
                    ...
                    /* CMS EDIT START - changing img for reattach helper */
                    var definition = self.getReattachHelperGraphicRepresentation(floatingAnchor, { paintStyle: self.paintStyle, source: nE });
                    definition.anchors = floatingAnchor;
                    floatingEndpoint = _newEndpoint(definition);
                    /* CMS EDIT END */
                    ...
                    if (jpc == null) {
                        ...
                        $cmsj(jpc.canvas).addClass("NewConnection");
                        jpc.endpoints[0].node.toggleClass("NewConnection");
                        $cmsj(inPlaceCopy.canvas).addClass("NewConnection");
                    ...
                    $cmsj(".tooltip").hide(); // CMS EDIT - tooltip support
                };
            ...
            this.initDropTarget = function(canvas) { //CMS EDIT needed to be public because of dropping to nodes
                if (params.isTarget && jsPlumb.CurrentLibrary.isDropSupported(_element)) {
                    ...
                    var drop = function() {
                        ...
                        if (!usedEndpoint.isFull() && !(idx == 0 && !usedEndpoint.isSource) && !(idx == 1 && !usedEndpoint.isTarget) && (usedEndpoint.elementId != jpc.endpoints[ oidx ].elementId || _currentInstance.Defaults.AllowNodeAttachToSelf)) { //CMS EDIT: check if connecting one node to itself
                            ...
                            if (!jpc.suspendedEndpoint) {
                                ...
                                $cmsj(jpc.canvas).removeClass("NewConnection"); // CMS EDIT - bringing back connection
                            }
                            else {
                    ...
                    dropOptions[dropEvent] = _wrap(dropOptions[dropEvent], function(event, ui) { 
                        ...
                        if($cmsj(this).data("nodeObject")) {
                            jsPlumbInstance = $cmsj(this).data("nodeObject").myJsPlumb;
                        } else {
                            jsPlumbInstance = $cmsj(this).data("jsPlumbObject")._jsPlumb;
                        }
    ...
    jsPlumb.Endpoints.Image = function(params) {
        ...
        self.canvas.onload = function() {
            ...
            if(canvasJQ.hasClass("jsPlumb_floating")){
                var position = $cmsj(".ui-draggable-dragging").position();
                if(position){
        ...
        this.resize = function(){
            $cmsj(self.canvas).css("width","");
            $cmsj(self.canvas).css("height","");
            $cmsj(self.canvas).removeAttr("width","");
            $cmsj(self.canvas).removeAttr("height","");
        ...
        this.paint = function(d, style, anchor) {
            if (self.ready) {
                ...
                }, 10); // CMS EDIT - better response time
            }
        };				
    };
...
;(function() {
    ...
    VmlConnector = function(params) {
        ...
        this.paint = function(d, style, anchor) {
            if (style != null) {				
                ...
                if (self.canvas == null) {										
                    ...
                    self.attachListeners(self.canvas, self);
                    // CMS EDIT - mouseDown detection
                    $cmsj(self.canvas).mousedown(function (self) { return function () { self.mouseDown(); } }(self))
                        .mouseup(function (self) { return function () { self.releaseMouse(); } }(self))
                }
                else {
                    p["coordsize"] = (d[2] * scale) + "," + (d[3] * scale);
                    _pos(self.canvas, d);
        ...
        /* CMS EDIT - mouse down detecting support */
...
// ********************************* CANVAS RENDERERS FOR CONNECTORS AND ENDPOINTS *******************************************************************
    ...
    var CanvasMouseAdapter = function() {
        ...
        this.mousemove = function(e) {		 
            ...
            else if (_mouseover && (!over || !_continue) && !eventSourceWasOverlay && !self.isMouseDown()) {
                _mouseover = false;
                self.fire("mouseexit", self, e);	
                return true;			
            }
            
            if (over || _mouseover) { // CMS EDIT - unnecesarry mousemove event triggering
...
(function($) {	
    ...
    jsPlumb.CurrentLibrary = {					        
        ...
        setDraggable : function(el, draggable) {
            el.draggable({ disabled: !draggable });
        },
