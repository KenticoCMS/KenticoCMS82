...
public partial class CMSModules_Content_Controls_Dialogs_Properties_CopyMoveLinkProperties : ContentActionsControl
{
    ...
    private void LinkDoc(object parameter)
    {
        if ((nodeIds.Count > 0) && (targetId != 0))
        {
            ...
            List<int> currentNodeIds = new List<int>();
        ...
        finally
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (node != null)
                    {
                        ctlAsyncLog.Parameter = "SelectNode(" + node.NodeID + ");RefreshTree(" + node.NodeID + ", " + node.NodeID + ");";
                    }
                }
            }
        }
    ...
    {
        LogContext.LogEvent(EventType.ERROR, "Content", eventCode, EventLogProvider.GetExceptionLogMessage(ex),
