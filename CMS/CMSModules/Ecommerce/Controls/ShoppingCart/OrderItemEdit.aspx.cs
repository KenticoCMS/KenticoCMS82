...
public partial class CMSModules_Ecommerce_Controls_ShoppingCart_OrderItemEdit : CMSEcommercePage
{
    ...
    private bool EditingEnabled
    {
        get
        {
            ...
            return ((Order != null) && !Order.OrderIsPaid) && ECommerceSettings.EnableOrderItemEditing;
        }
    }
    ...
    private void EditForm_OnItemValidate(object sender, ref string errorMessage)
    {
        ...
        // Check the price of order item
        if (ctrl.FieldInfo.Name == "CartItemUnitPrice")
        {
            if (ValidationHelper.IsDouble(ctrl.Value))
            {
                bool isDonation = SKU.SKUProductType == SKUProductTypeEnum.Donation;

                // Get new price of the item, which is set in the shopping cart currency
                double newPrice = ValidationHelper.GetDouble(ctrl.Value, 0.0);

                // Get the range in the shopping cart currency
                ...
                // Check whether the item is donation and the new price is not in the range
                if (isDonation && (((minPrice > 0) && (newPrice < minPrice)) || ((maxPrice > 0) && (newPrice > maxPrice))))
                {
                    // Formatted range of the donation price with the currency codes
                    string fMinPrice = ShoppingCart.GetFormattedPrice(minPrice);
                    string fMaxPrice = ShoppingCart.GetFormattedPrice(maxPrice);

                    if ((minPrice > 0.0) && (maxPrice > 0.0))
                    {
                        // New price is not in the range 
                        errorMessage = String.Format(GetString("com.orderitemedit.pricerange"), fMinPrice, fMaxPrice);
                    }
                    else if (minPrice > 0.0)
                    {
                        // New price is lower than the minimal price
                        errorMessage = String.Format(GetString("com.orderitemedit.pricerangemin"), fMinPrice);
                    }
                    else if (maxPrice > 0.0)
                    {
                        // New price is higher than the max price
