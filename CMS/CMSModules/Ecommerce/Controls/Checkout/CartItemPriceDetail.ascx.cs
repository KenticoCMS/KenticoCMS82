...
public partial class CMSModules_Ecommerce_Controls_Checkout_CartItemPriceDetail : CMSCheckoutWebPart
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        script.Append("function ShowProductPriceDetail(cartItemGuid, sessionName, includeOptions) {");
        ...
        script.Append(string.Format("modalDialog('{0}?itemguid=' + cartItemGuid + sessionName + '&includeoptions=' + includeOptions, 'ProductPriceDetail', 750, 560); }}", ResolveUrl(detailUrl)));
    ...
    {
        if ((ShoppingCart.ContentTable != null))
        {
            return string.Format("<img src=\"{0}\" onclick=\"javascript: ShowProductPriceDetail('{1}', '{2}', '{4}')\" alt=\"{3}\" class=\"ProductPriceDetailImage\" style=\"cursor:pointer;\" />",
