...
public partial class CMSModules_Ecommerce_Pages_Tools_Products_Variant_New : CMSProductsPage
{
    ...
    protected override void OnPreInit(EventArgs e)
    {
        ...
        mAllCategoriesOptions = new Dictionary<OptionCategoryInfo, List<SKUInfo>>();
        mVariantCategoriesOptions = new Dictionary<OptionCategoryInfo, List<SKUInfo>>();
        ...
        // Fill dictionaries
        FillCategoriesOptionsDictionary(mAllCategoriesOptions, allCategoriesDS);
        FillCategoriesOptionsDictionary(mVariantCategoriesOptions, variantCategoriesDS);

        // Pass data to controls
        VariantFilter.ExternalDataSource = mAllCategoriesOptions;
        CategorySelector.AllCategoriesOptions = mAllCategoriesOptions;
        CategorySelector.VariantCategoriesOptions = mVariantCategoriesOptions;
    }
        ...
        switch (e.CommandName.ToLowerCSafe())
        {
            ...
            case "save":
                ...
                if ((NewCategories.Count > 0) || (ExistingCategories.Count > 0))
                {
                    ...
                    var selectedItems = VariantGrid.SelectedItems.ToHashSet();
    ...
    {
        foreach (OptionCategoryInfo optionCategory in mVariantCategoriesOptions.Keys)
        {
            if (!existingCategories.Contains(optionCategory.CategoryID))
            {
                existingCategories.Add(optionCategory.CategoryID);
            }
        }
    }
