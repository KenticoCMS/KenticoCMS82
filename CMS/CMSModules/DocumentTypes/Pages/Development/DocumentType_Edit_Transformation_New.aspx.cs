...
public partial class CMSModules_DocumentTypes_Pages_Development_DocumentType_Edit_Transformation_New : CMSDesignPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!CurrentUser.IsAuthorizedPerUIElement("CMS.Design", new[] { "Design", "Design.WebPartProperties", "WebPartProperties.General", "WebPartProperties.NewTransformations" }, SiteContext.CurrentSiteName))
        {
            RedirectToUIElementAccessDenied("CMS.Design", "Design.WebPartProperties;WebPartProperties.General;WebPartProperties.NewTransformations");
        }
    ...
    {
        var tinfo = editElem.EditedObject as TransformationInfo;
        if (tinfo != null)
        {
            tinfo.TransformationClassID = (ClassID > 0) ? ClassID : filter.ClassId;
        }
    }
