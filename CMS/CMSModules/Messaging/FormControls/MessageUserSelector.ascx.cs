...
public partial class CMSModules_Messaging_FormControls_MessageUserSelector : FormEngineUserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            string url = AuthenticationHelper.ResolveDialogUrl("~/CMSModules/Messaging/Dialogs/MessageUserSelector_Frameset.aspx");
            if (IsLiveSite)
            {
                if (!MembershipContext.AuthenticatedUser.IsPublic())
                {
                    url = AuthenticationHelper.ResolveDialogUrl("~/CMSModules/Messaging/CMSPages/MessageUserSelector_Frameset.aspx");
                }
                else
                {
                    url = AuthenticationHelper.ResolveDialogUrl("~/CMSModules/Messaging/CMSPages/PublicMessageUserSelector.aspx");
                }
            }
