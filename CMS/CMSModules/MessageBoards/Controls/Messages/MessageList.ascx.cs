...
public partial class CMSModules_MessageBoards_Controls_Messages_MessageList : CMSAdminListControl
{
    ...
    private void InitializeGroupNewMessage()
    {
        ...
        action.OnClientClick = "modalDialog('" + AuthenticationHelper.ResolveDialogUrl("~/CMSModules/Groups/CMSPages/Message_Edit.aspx") + "?boardId=" + BoardID + "&groupid=" + GroupID + "&changemaster=" + QueryHelper.GetBoolean("changemaster", false) + "', 'MessageEdit', 800, 535); return false;";
    ...
    public void ReloadFilter()
    {
        if (GroupID == 0)
        {
            plcSite.Visible = MembershipContext.AuthenticatedUser.IsGlobalAdministrator;
        }
