...
public partial class CMSModules_AdminControls_Controls_Class_Layout : CMSUserControl
{
            ...
            if (mOldObjectId != ObjectID)
            {
                ...
                if (!IsAlternative)
                {
                    switch (FormLayoutType)
                    {
                        ...
                        case FormLayoutTypeEnum.Bizform:
    ...
    public ObjectEditMenu ObjectEditMenu
    {
        get
        {
            if (mObjectMenu == null)
            {
                mObjectMenu = ControlsHelper.GetChildControl(Page, typeof(ObjectEditMenu)) as ObjectEditMenu;
            }
            return mObjectMenu;
        }
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            pnlObjectLocking.ObjectManager.OnBeforeAction += ObjectManager_OnBeforeAction;
    ...
    /// </summary>
    public bool IsEditedObjectLocked()
    {
    ...
    /// </summary>
    protected void InitASCXeditor()
    {
        txtCode.Editor.Language = LanguageEnum.ASPNET;
        txtCode.Editor.Height = new Unit("300px");

        //ScriptHelper.RegisterStartupScript(this, typeof(String), "FullScreenInit", "setTimeout('if (CM_instances != null) {CM_instances[0].toolbar.fitWindow();}', 50)", true);

        string lang = DataHelper.GetNotEmpty(SettingsHelper.AppSettings["CMSProgrammingLanguage"], "C#");
        lblDirectives.Text = string.Concat("&lt;%@ Control Language=\"", lang, "\" AutoEventWireup=\"true\" Inherits=\"CMS.FormControls.CMSAbstractFormLayout\" %&gt;<br />&lt;%@ Register TagPrefix=\"cms\" Namespace=\"CMS.FormControls\" Assembly=\"CMS.FormControls\" %&gt;");
    }
    ...
    private void FillFieldTypeList()
    {
        ...
        if (drpLayoutType.SelectedValue == LayoutTypeEnum.Html.ToString())
        {
            ...
            {
                DataClassInfo dci = DataClassInfoProvider.GetDataClassInfo(DataClassID);
                bool visible = (dci != null) && (dci.ClassName.EqualsCSafe("cms.user", StringComparison.InvariantCultureIgnoreCase));
                drpFieldType.Items.Add(new ListItem { Text = "documenttype_edit_form.visibilitycontrol", Value = "visibility", Enabled = visible });
            }
