...
public partial class CMSModules_SocialMarketing_FormControls_LinkedInAutoPost : SocialMarketingAutoPostControl
{
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        if (StopProcessing)
        {
            ...
            {
                ShowPostStateWarning(ResHelper.GetString("sm.linkedin.posts.noreadpermission"));
            }
    ...
    private void ShowPostPublishState(LinkedInPostInfo post)
    {
        ...
        if (post.IsFaulty)
        {
            ...
            ShowError(String.Format("<strong>{0}</strong>: {1}", GetString("sm.linkedin.autopost"), message));
        }
