...
public partial class CMSModules_ContactManagement_FormControls_AccountStatusDialog : CMSModalPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        var siteName = mSiteID > 0 ? SiteInfoProvider.GetSiteName(mSiteID) : SiteContext.CurrentSiteName;
        var allowGlobal = SettingsKeyInfoProvider.GetBoolValue(siteName + ".cmscmglobalconfiguration");

        if ((mSiteID <= 0) && !allowGlobal)
        {
        ...
        if (mSiteID > 0)
        {
            gridElem.WhereCondition = "AccountStatusSiteID = " + mSiteID;

            // Check if global config is allowed for the site
            if (allowGlobal)
            {
                // Add account statuses from global configuration
                gridElem.WhereCondition = SqlHelper.AddWhereCondition(gridElem.WhereCondition, "AccountStatusSiteID IS NULL", "OR");
            }
        }
        else if ((mSiteID <= 0) && allowGlobal)
        {
