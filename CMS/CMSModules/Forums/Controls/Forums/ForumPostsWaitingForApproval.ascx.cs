...
public partial class CMSModules_Forums_Controls_Forums_ForumPostsWaitingForApproval : CMSAdminListControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else if (!MembershipContext.AuthenticatedUser.IsGlobalAdministrator)
        {
        ...
        // Hide approvals
        if ((!MembershipContext.AuthenticatedUser.IsGlobalAdministrator) && (String.IsNullOrEmpty(forumIDs)) && (hasGroupRights == false))
        {
        ...
        if (MembershipContext.AuthenticatedUser.IsGlobalAdministrator || hasGroupRights)
        {
            if (groupWhere != String.Empty)
            {
